...
const SCENES = [
  ...
  //   月給30万・賞与なし → 賃金日額 300,000×6÷180 = **10,000円**
  //   67%: 10,000×30×0.67 = 201,000円/月 ／ 50%: 10,000×30×0.50 = 150,000円/月
  //   365日 → 201,000×6 + 150,000×6 + 25,000 = **2,131,000円**
  //   13%は**配偶者が育休を取らないので0円**(既定)。**その理由が画面に出ること**まで見る
  //   (黙って0円にすると、配偶者が14日取れば36,400円もらえたことに永久に気づけない)。
  { name: "ikuji", expect: (s) =>
      s.daily === 10000 && s.ikujiTotal === 2131000 && s.total === 2131000 &&
  ...
  { name: "ikuji_shien", expect: (s) =>
      s.shien === 36400 && s.total === 2167400 && !s.failed },
  ...
  { name: "ikuji_cap", expect: (s) =>
      s.daily === 16110 && s.unit1 === 323811 && s.ikujiTotal === 1942866 &&
      s.showsCap && !s.failed },
  // 参照データが配信できないときは、金額を出さずに断る(fail closed)。
