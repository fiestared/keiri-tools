...
const SYNONYMS = [
  ...
  // ★以下は6人レビュー(2026-08-03)の実測で「答えがあるのに検索が MISS を返した」語。
  //   実装済み機能に届かない検索は、検索が無いより悪い(利用者は「このサイトには無い」と確信して離脱する)。
  //   when は base(title+answer) に実在する語だけを使う。「報酬」単体は標準報酬月額に当たるので使わない。
  { when: ["士業", "原稿料"], add: ["司法書士", "税理士 報酬", "弁護士 報酬", "行政書士", "土地家屋調査士", "外注 源泉"] },
  // 「随時改定」「月額変更届」は定時決定の記事にも出てくる(対比で必ず併記される)ので、
  // when に使うと「月変」が定時決定側にも付いて**逆の記事が上位に来る**。実測で確認済み。
  // 随時改定にしか無い語(固定的賃金・昇給降給)で釣る。
  { when: ["固定的賃金", "昇給・降給"], add: ["月変", "げっぺん", "昇給 保険料", "等級 変わる", "随時改定"] },
  { when: ["定時決定", "算定基礎"], add: ["算定", "算定基礎届", "4月 5月 6月 平均"] },
  { when: ["先方負担", "手数料差引"], add: ["消込", "入金消込", "入金が足りない", "入金 少ない", "差額 手数料"] },
