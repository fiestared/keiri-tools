...
**Bing 2,000/日 は到達シナリオを超えている（承知のうえで置いている）**
表示 15,000/日 × CTR10% ＝ **約1,500/日**が目安。★これは**シナリオであって在庫から確定した
天井ではない**（上位一部クエリのCTRを全クエリへ移せる根拠は無い。1〜3位帯のCTRは最新ラベルで
9.47%→8.06%に低下）。2,000に届かせるには **表示自体が 20,000/日 へ増える**必要があり、
これは外部要因＝自力レバーではない。→ **2026-12-31 に中間確認**し、表示7日平均が 18,000/日 に
届いていなければ Bing 目標を 1,500 へ下げる。
...
なぜ Google を別 KPI にするか: 流入の9割が Bing で、上のとおり Bing 単独では天井がある。
日本の検索シェアは Google が Bing の 6〜8 倍なので、伸びしろの主役は Google 側にある。
定数は `build.mjs` の `SESSION_GOAL` / `BING_GOAL` / `NONSEARCH_GOAL` /
`BING_CEILING_NOW` / `GOOGLE_KPI`。
