...
## 目標と Google トラック KPI（2026-09-07 追加）

タイルの直下に **「目標: 日 10,000 セッション」** の欄がある。Masahiro の「日1万セッション狙おうぜ」を受けて置いた。

- **1日セッション**（GA4・直近7日平均・昨日まで）と目標 10,000 に対する達成率
...
- 通過点: 10月末 30 / 12月末 100 / 2027-03末 1,000（クリック/日・7日平均で判定）。
  2026-09-07 の分析で置いた**仮の値**。外れたら数字だけ動かさず方針ごと見直す
- 28日ぶんのクリック（棒）と表示（線）のグラフ。表示と順位はクリックより先に動く先行指標

なぜ Google を別 KPI にするか: 流入の9割が Bing で、Bing の表示は平日1.9万/日で頭打ち。
全部 1〜3 位でも天井は約 1,800/日。日本の検索シェアは Google が Bing の 6〜8 倍なので、
日1万は実質「Google で Bing 並みの順位を取る」目標になる。定数は `build.mjs` の
`SESSION_GOAL` / `GOOGLE_KPI`。
