...
- **リピーター率は人数基準**（GA4 `newVsReturning` の `activeUsers`）。マーケで言う新規／リピートの比は
  人数の比で、セッション基準とは別物（リピーターは1人で何セッションも作るので必ず高く出る）。
  分母は「新規＋リピート」
- 🔴 **窓ごとに別クエリで取る。日別の人数は足さない** — 同じ人が翌日また来れば2人に数えられるので、
  「今日＋昨日」は「直近7日」にならない。`dateRanges` に窓を並べると GA4 が `dateRange` 次元を
  足して返すので、1リクエストで窓ごとの重複排除済みの数字が取れる
  （gbrain `keiri-tools/x-referral-ga4-baseline-2026-08-28`）
- 🔴 **`dateRanges` は1リクエストに4本まで**（5本目で 400 `Requests are limited to 4 dateRanges.`
  2026-09-21 実測）。画面に出す窓は6つ（今日・昨日・直近7日・その前の7日・直近4週・その前の4週）なので、
  **2リクエストに分けてある**（`nvr` と `nvrMonth`）。窓を足す時はこの上限に当たる
- **月の窓は30日ではなく28日＝4週ちょうど。** 30日にすると窓ごとに土日の本数が変わり、
  前の窓との差が曜日の偏りで動く（このサイトは土日で平日の3〜4割まで落ちる）
- **その窓に訪問が1件も無い時は「0%」ではなく「まだ取れていない」と出す。** 道しるべは
  公開が 2026-09-07 なので、前の4週には行そのものが返ってこない
- 「判別なし」は次元値が**空**の行（当日まだ判別が付いていない）と `(not set)` の合計。率の分母から
  外し、件数だけ画面に出している。2026-09-21 実測で keiri の当日は 71セッション中28が空で、
  昨日以前は空ゼロ。**当日の率は途中の数字**
- リピーターの**1人あたりセッション数**を必ず併記する。率だけ見ると読者が定着したように見えるが、
  少人数が何度も開いているだけのことがある（自分の確認アクセスを含む。道しるべは
  gbrain `findings/pachisloshirube-acquisition-local-traffic-caveat-2026-09-08` で実測済み）
- Cookie 基準なので、別の端末・別のブラウザ・Cookie を消した再訪は**新規に戻る**。
  リピーター率は「下限」であって実際の再訪率ではない
