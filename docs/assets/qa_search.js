...
/**
 * 被覆の分子から外す語の df 割合。①本文(title+answer)に無く、かつ②索引のこの割合を超えて
 * 出る語は、話題を特定していないとみなす。**②だけでは切らない**（上の docstring 参照）。
 *
 * 🔴 **この値は索引が増えると勝手に跨がれる（2026-08-26 に実際に跨がれた）。**
 *   直上の docstring は「MIN_CONTENT_COVERAGE は索引が増えても勝手に動かない」と書いており
 *   それは正しいが、**その分子から何を外すかを決めるこの門は、索引の件数を分母に持つ**。
 *   実測: 記事を1本足して索引が 374→375 になった瞬間、`目安` の df が
 *   **40.11% → 40.00%** に落ち、`> 0.4` が false になって門が開いた。
 *   結果 `資格の勉強時間の目安` → /santei/ が被覆を通り、best 0.000 → 7.330 で matched になった
 *   （tests/test_qa.mjs の無関係クエリ検査が赤）。
 *   ★ **記事の中身とは無関係**で、375本目が何であっても同じことが起きた。
 *
 *   旧値 0.4 は索引が114件だった 2026-08-08 の較正で、当時 `目安` は **70%** だった。
 *   同義語は「計算」を含むページにしか撒かれないので、記事が増えるほど**割合は下がり続ける**。
 *   ＝ この門は**放っておくと必ず下から跨がれる向き**にドリフトする。
 *
 *   0.25 にした根拠（2026-08-26 実測・索引375件）:
 *     撒かれる同義語  いくら 51.47% / 金額 48.80% / 目安 40.00% /
 *                     どのくらい 39.73% / いくらぐらい 39.73%  ← 群の最小が 39.73%
 *     本文の内容語    計算 50.40%(本文135件) / 期限 12.27% / 税率 6.13%
 *   ★ `どのくらい` と `いくらぐらい` は **0.4 では既に外れていなかった**＝門は
 *     今日より前から部分的に効いていなかった。群の最小 39.73% から 15pt 下げて 0.25 とする。
 *   ★ 内容語は①で守られるので、この値を下げても「計算」は落ちない（df だけで切ると落ちる、
 *     という 2026-08-08 の警告は**②単独で切った場合**の話）。
 *   ✅ ドリフトは `tests/test_qa_stopword_drift.mjs` が測って落とす。この値を手で見張らない。
 */
const COVERAGE_STOPWORD_DF_RATIO = 0.25;
const HIRAGANA = /[ぁ-ゖ]/;
