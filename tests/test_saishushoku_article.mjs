...
if (oldBox) {
  ...
  // ★★2026-08-14 追加: 差の割合が、このページの数字から導けること。
  //   本文は「約3%少なく見積もります」と書いていたが、
  //   (6,745−6,395)/6,745 = 5.2% ／ (5,454−5,170)/5,454 = 5.2% で **3%はどこからも出ない**
  //   （前年度の差の値が残ったと思われる）。他所から持ってきた数字を断定に使わせない。
  const m = t.match(/金額を約(\d+)%少なく見積もります/);
  ok(!!m, "古い上限との差の割合の記述が消えている");
  if (m) {
    const actual = Math.round(((6745 - 6395) / 6745) * 100);   // = 5
    ok(Number(m[1]) === actual,
      `差が「約${m[1]}%」だが、記事の数字から出るのは約${actual}%（(6,745−6,395)/6,745）`);
  }
}
