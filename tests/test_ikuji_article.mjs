...
// ── 前提(厚労省リーフレットが明示している額。LL080731保02・令和9年7月31日まで) ──
const CAP = 16540;        // 休業開始時賃金日額の上限額
const FLOOR = 3203;       // 同 下限額(下の外部オラクルで検証する)
const R67 = 0.67, R50 = 0.50, R13 = 0.13;

// 月給別の早見表に載せている月給(2.5万円刻み)。生成側 tools/gen_ikuji_table.mjs の WAGES と同じ。
// ★意図して二重に書いている: ここが生成側を import すると、生成側が壊れたときに
//   検査も一緒に壊れて「常に一致する」空虚な検査になる。ズレたら網①が両方向から落とす。
...
const oracle = [
  ["育休67%上限", amount(CAP, 30, R67), 332454],
  ["育休50%上限", amount(CAP, 30, R50), 248100],
  ["出生時育休67%上限", amount(CAP, 28, R67), 310290],
  ["出生後支援13%上限", amount(CAP, 28, R13), 60205],
  ["育休67%下限", amount(FLOOR, 30, R67), 64380],
  ["育休50%下限", amount(FLOOR, 30, R50), 48045],
  ["出生後支援13%下限", amount(FLOOR, 28, R13), 11658],
];
...
const derived = new Set([
  ...
  323811,                              // 改定前(令和8年7月31日まで)の67%上限額。改定の事実として記載
  // ── 月給別の早見表(tools/gen_ikuji_table.mjs が ikuji_core から生成している) ──
  // ★ここでは**コアを呼ばない**。この検査の存在意義は「記事の数字をコアとは独立に検算する」
  //   ことなので、コアを import した瞬間に「コアが間違っていたら記事も一緒に間違う」を
  //   見逃す検査になる。上の amount()/dailyWage() は61条の7第6項から起こした別実装。
  ...HAYAMIHYO_WAGES.flatMap((w) => {
    const d = Math.min(Math.max(dailyWage(w), FLOOR), CAP);
    return [w, amount(d, 30, R67), amount(d, 30, R50)];
  }),
]);
...
const pctExpected = new Set([
  67, 50, 13, 80,                                           // 給付率
  Math.round(amount(d50, 30, R67) / M50 * 1000) / 10,       // 66.5% (上限に当たる人の実質率)
...
const DAYS_EXPECTED = new Map([
  ...
  // 母親は産後休業(出生日の翌日から8週=56日)に続けて育休に入るため、起算日が出生日ではない。
  // 出生日 + 1(翌日から) + 56 = 出生日から58日目。厚労省PL070801保03が「58日目に当たる日」と明記。
  // ★この日を出生日と取り違えると、初回申請の提出期限(育休開始から4か月経過月の末日)がまるごとずれる。
  [56 + 1 + 1, "母親の「育児休業開始日」= 出生日から起算して58日目(産後休業56日の後)"],
]);
...
const expiryCallout = callouts.find((c) => c.includes("2027年8月1日に変わります"));
ok(!!expiryCallout, "網の外④: 上限額の改定予告calloutが見つからない");
if (expiryCallout) {
  ok(expiryCallout.includes("令和9年（2027年）7月31日まで"),
     "網の外④: 上限額の有効期限(令和9年7月31日まで)が明示されていない");
  ok(expiryCallout.includes("毎年8月1日に改定"),
     "網の外④: 毎年8月1日改定という仕組みが書かれていない");
}
// 本文側にも有効期限が付いているか(上限額を裸で書いていないか)
ok(/16,540円[^。]{0,40}令和9年7月31日までの額/.test(text) || /16,540円/.test(text) && /令和9年7月31日までの額/.test(text),
   "網の外④: 上限額16,540円に有効期限の断りが付いていない");
