...
const BREAKS = [
  ...
  // ── 第8便（§42）: /kihonteate/ が「受給期間は1年」とだけ書き、20条1項の延長（最大4年）を画面に出していなかった ──
  // ★いちばん大事なのは「育児」を消す壊し。1年の育休をとる人はその1年ずっと求職活動できず、
  //   延長を知らないと **1日ももらえないまま期限切れ**になる（¥満額 → ¥0）。
  [KH, "★受給期間延長から「育児」を消す（1年育休の人が全額あきらめる）", (h) => h.replace("<li><b>育児</b>のため（3歳未満の子を養育するときなど）</li>", "")],
  [KH, "受給期間延長から「妊娠」を消す", (h) => h.replace("<li><b>妊娠</b>したとき</li>", "")],
  [KH, "受給期間延長から「病気・ケガ」を消す", (h) => h.replace(/<li><b>病気・ケガ<\/b>のため（疾病・負傷[^<]*）<\/li>/, "")],
  [KH, "「受給期間を最大4年まで延長」を消す", (h) => h.replace("受給期間を最大4年まで延長", "受給期間を延長")],
  [KH, "★「申し出ないと延長されない」を消す（申請主義）", (h) => h.replace("申し出ないとされません", "延長されます")],
  [KH, "受給期間延長のブロックごとidを外す", (h) => h.replace('id="jukyu-kikan-encho"', 'id="gone-jk"')],

  // ── 同じ /kihonteate/ の「勤続1年未満は原則もらえません」も、13条1項かっこ書きの救済（2年→最大4年）を隠していた ──
  [KH, "算定対象期間の延長から「出産」を消す", (h) => h.replace("<li><b>出産</b></li>", "")],
  [KH, "算定対象期間の延長から「事業所の休業」を消す", (h) => h.replace("<li><b>事業所の休業</b></li>", "")],
  [KH, "「さかのぼる窓を最大4年まで」を消す", (h) => h.replace("さかのぼる期間を最大4年まで", "さかのぼる期間を")],
  [KH, "算定対象期間の救済ブロックごとidを外す", (h) => h.replace('id="santei-taisho-encho"', 'id="gone-st"')],
];
