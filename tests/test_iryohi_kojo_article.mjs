...
// (7) ★所得税を払っていない人に寄せても戻らない（記事の落とし穴。callout の中の <p> を名指し）
{
  ...
  if (!zei0) fail('「税金を払っていない人に寄せても戻らない」の callout が無い');
    ...
    // ★2026-08-13 改訂: 「所得税の還付はゼロ」という**文言の完全一致**を見ていた。
    //   記事を「医療費控除は実際に支払った人が控除する（申告時に付け替えられない）」へ
    //   直したとき、同じ意味を「還付は生じない」と書き換えただけで**赤になった**。
    //   検査したいのは文言ではなく、**納税していない人に負担させても戻らない**という警告が
    //   残っていること。表現の言い換えで落ちる検査は、正しい改稿を妨げる。
    if (p && /(還付はゼロ|還付は生じない|還付されません|1円も戻ら)/.test(p))
      ok('落とし穴: 納税していない人が負担しても還付は生じない');
    else fail('★「納税していない人が負担しても還付は生じない」の警告が失われている');
  }
}
...
{
  const co = [...body.matchAll(/<div class="callout">([\s\S]*?)<\/div>/g)].map(m => m[1]);
  // 2026-09-23 書き直し。旧版は「現行法のままなら今年（令和8年分）が最後の適用年」と書き、検査もそれを要求していた。
  // しかし令和8年法律第12号で措法41条の17は改正済み（令和9年分以後に適用＝同法附則46条）。
  //   e-Gov法令API v2 law_data/332AC0000000026_20270101_508AC0000000012 第41条の17第1項:
  //     第1号（第2項第1号の医薬品である一般用医薬品等。薬局製造販売医薬品を除く）「平成二十九年一月一日以後の期間」
  //     第2号（それ以外）「平成二十九年一月一日から令和十三年十二月三十一日までの期間」
  //   附則46条（sochi 332AC0000000026 の SupplProvision 令和八年三月三一日法律第一二号）
  //     「新租税特別措置法第四十一条の十七の規定は、令和九年分以後の所得税について適用し、…」
  // ツールのデータ docs/assets/iryohi_r08.json の selfmed.kigen と同じ内容。
  const self = co.find(c => c.includes('令和8年法律第12号'));
  if (!self) fail('セルフメディケーション税制の期限（令和8年法律第12号による改正）を述べた callout が無い');
  else {
    const t = strip(self);
    if (t.includes('平成29年1月1日から令和8年12月31日までの間')) ok('セルフメディケーション: 令和8年分の期間は令和8年12月31日まで');
    else fail('★令和8年分の期間（平成29年1月1日から令和8年12月31日までの間）が改ざんされている');
    if (/令和9年分以後の所得税に適用/.test(t) && /附則46条/.test(t)) ok('セルフメディケーション: 改正は令和9年分以後に適用（附則46条）');
    else fail('★改正の適用時期（令和9年分以後・附則46条）が無い/違う');
    if (/スイッチOTC医薬品（薬局製造販売医薬品を除く）は「平成29年1月1日以後の期間」で、期限がありません/.test(t))
      ok('セルフメディケーション: スイッチOTC（薬局製造販売医薬品を除く）は期限なし');
    else fail('★スイッチOTCの「期限なし」が無い/改ざんされている');
    if (/それ以外（[^）]*）は令和13年12月31日まで/.test(t)) ok('セルフメディケーション: それ以外は令和13年12月31日まで');
    else fail('★スイッチOTC以外の期限（令和13年12月31日まで）が無い/改ざんされている');
    if (/12,000円・88,000円の金額は改正後も変わりません/.test(t)) ok('セルフメディケーション: 金額は改正後も据え置き');
    else fail('改正後も12,000円・88,000円が変わらない旨が無い');
  }
  // ★旧主張が戻ったら赤: 「令和8年分で終わる」「令和8年分が最後」「延長される可能性」（ページ全体・head含む）
  {
    const all = strip(html);
    const bad = [/令和8年分で終わる/, /令和8年分が最後/, /今年が最後/, /令和8年（2026年）分が最後/, /延長される可能性/]
      .filter(re => re.test(all)).map(String);
    if (bad.length) fail(`★成立済みの改正と食い違う旧主張が残っている: ${bad.join(' ')}`);
    else ok('セルフメディケーション: 「令和8年分で終わる」系の旧主張がページに無い');
    const h2 = (html.match(/<h2 id="self">([\s\S]*?)<\/h2>/) || [])[1] || '';
    if (/令和9年分以後も続く/.test(h2)) ok('h2: セルフメディケーション税制は令和9年分以後も続く');
    else fail('★セルフメディケーションの h2 が「令和9年分以後も続く」でない');
  }
