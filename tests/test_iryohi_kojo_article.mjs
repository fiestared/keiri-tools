...
// 国税庁 No.1410「給与所得控除」令和8年分（収入660万円以下の範囲。記事の表はこの範囲だけを使う）
const kyuyoKojo = R => {
...
const oracle = ROWS.map(R => {
  const kojo = kyuyoKojo(R);
  const shotoku = R - kojo;
...
// 境目: 足切りが10万円を下回る最大の給与収入 = 2,971,427円（記事が名指ししている数）
const KYOKAI = (() => {
  for (let R = 2_980_000; R >= 2_960_000; R--) {
    if (ashikiri(R - kyuyoKojo(R)) < ASHIKIRI_CAP) return R;
  }
  throw new Error('境目が見つからない');
})();
if (KYOKAI === 2_971_427) ok(`オラクル: 足切りが10万円を下回る最大の給与収入 = ${KYOKAI.toLocaleString()}円`);
else fail(`オラクル: 境目の給与収入が 2,971,427 でなく ${KYOKAI}`);
...
// (1b) ★給与所得控除の式は「本文」と「出典」の2か所に出るので、網では守れない（規則7）。
//      本文だけ「30％＋8万円」→「30％＋10万円」に壊しても、8万円は出典に残り10万円は元からある
//      ＝集合は変わらず素通しする（壊しテストで実際に素通しした）。→ 境目の段落を名指しする。
{
  const p = [...body.matchAll(/<p>([\s\S]*?)<\/p>/g)].map(m => strip(m[1]))
    .find(t => t.includes('境目は') && t.includes(KYOKAI.toLocaleString()));
  if (!p) fail('境目（総所得金額等200万円）を述べた段落が見つからない');
  else {
    if (p.includes('収入×30％＋8万円')) ok('境目の段落: 給与所得控除の式が「収入×30％＋8万円」');
    else fail('★境目の段落の給与所得控除の式が国税庁No.1410と違う');
...
//                   ◯✕の割り当て → (4)(5)(6) の要素照合で守る
{
  ...
  const kaneWant = [...new Set([
    ...
    // 境目（オラクル由来）
    KYOKAI.toLocaleString() + '円', '1,999,999円', '99,999円',
    // 還付の例（オラクル由来）
    '20,000円', '20,420円', '40,420円', '40,840円', '60,840円', '4,000円',
    // リード・FAQ の例（年収160万円・医療費6万円 → 60,000−47,500）
    '12,500円',
    // 速算表（国税庁No.2260）
