...
// ─────────────────────────────────────────────────────────
{
  ...
  // FAQの上限例は表と同じ式（5万円×20%＋復興2.1%＋住民税2,500円＝12,710円）
  ok("FAQ: 上限の節税額は内訳合計の12,710円", /年12,710円/.test(HTML));
  ok("FAQ: 誤った12,550円を出していない", !/12,550/.test(HTML));
  ok("FAQ: 税率10%の例は7,605円", /年7,605円/.test(HTML));
  ok("FAQ: 誤った8,105円を出していない", !/8,105/.test(HTML));
