...
const MUT = [
  ...
  [CORE, "if (!(standardMonthly > 0)) return null;   // 分からないものを黙ってウに落とさない",
         "if (false) return null;", "標報不明を黙って区分ウに落とす"],
  [CORE, "if (standardMonthly >= 830000) return by(\"a\");", "if (standardMonthly > 830000) return by(\"a\");",
         "区分アの境目を「以上」→「超」にする"],
