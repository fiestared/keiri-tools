...
console.log("── 待期は「通算7日」・認定は4週に1回で直前28日分 ──");
{
  ...
  const t = strip(seg);
  const need = ["通算して7日", "21条", "4週間に1回", "28日", "15条3項", "15条4項", "15日未満"];
  const missing = need.filter((x) => !t.includes(x));
...
idHas("nintei-28", ["4週間に1回", "まとまるのは28日分", "待期の7日と、最初の認定日までの4週間", "1か月前後"],
  "p 1回の認定でまとまるのは28日分・初回入金まで1か月前後");

console.log("── 12条の逐語（blockquote に条文本文だけ。出典ラベルを同居させない） ──");
{
  const bqs = [...HTML.matchAll(/<blockquote>([\s\S]*?)<\/blockquote>/g)].map((m) => strip(m[1]));
  const want = "租税その他の公課は、失業等給付として支給を受けた金銭を標準として課することができない。";
  const hit = bqs.filter((b) => b === want);
  if (hit.length !== 1) bad("12条の blockquote", `逐語一致が${hit.length}件（実際: ${JSON.stringify(bqs)}）`);
  else ok("12条（公課の禁止）を blockquote に逐語で置いている");
}
...
console.log("── 訓練延長給付と技能習得手当（競合の計算ページに無い一段） ──");
{
  ...
  const t = strip(seg);
  const need = ["指示", "訓練が終了する日まで", "500円", "20,000円", "42,500円", "10,700円", "傷病手当", "15日以上"];
  const missing = need.filter((x) => !t.includes(x));
