...
const rawOrig = { page: fs.readFileSync(PAGE, "utf8"), data: fs.readFileSync(DATA, "utf8") };
const orig = { ...rawOrig, page: rawOrig.page.replace(/<td class="num">/g, "<td>") };
const restore = () => { fs.writeFileSync(PAGE, rawOrig.page); fs.writeFileSync(DATA, rawOrig.data); };
...
const cases = [
  ...
  {
    // ★2026-08-07 修正: この壊し方は **head の JSON-LD に当たっていた**（editPage は
    //   indexOf ＝最初の1件だけ置換する。同じ文言が JSON-LD と本文の2箇所にある）。
    //   規則8「壊し方も一意に。head の JSON-LD に当てない」に反していた。
    //   ★そして当時、**JSON-LD を見る検査が1つも無かった**ので素通ししていた。
    //   いまは両方を検査するので、狙いを本文（#meisaisho）に定め直す。
    name: "⑨ #meisaisho の保存期間を「5年」→「3年」にする（No.1120と食い違う）",
    expect: "保存期間の起算・年数がNo.1120と違う",
    apply: () => editPage("確定申告期限等から5年を経過する日までの間</b>、領収書の提示または提出を求められることがあります",
                          "確定申告期限等から3年を経過する日までの間</b>、領収書の提示または提出を求められることがあります"),
  },
  {
    // ★JSON-LD（検索結果のリッチリザルトに出る）も本文と同じ数字を名乗る。
    //   本文だけ正しくても、ここが嘘なら読者は検索結果で嘘を読む。
    name: "⑨' head の JSON-LD だけ保存期間を「5年」→「3年」にする",
    expect: "JSON-LD の保存期間がデータ（5年）と違う",
    apply: () => editPage("確定申告期限等から5年を経過する日までの間、領収書の提示または提出を求められることがあるので",
                          "確定申告期限等から3年を経過する日までの間、領収書の提示または提出を求められることがあるので"),
  },
  {
