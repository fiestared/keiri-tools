 ...
 *
 *   使うオラクル（令和8年8月1日〜令和9年7月31日に適用される額。LL080731保02）:
 *     育児休業給付金 67% … 上限 **332,454円** ／ 下限 **64,380円**
...
eq(MAX, 16540, '賃金日額の上限（ハ＝30歳以上45歳未満）');
eq(MIN, 3203, '賃金日額の下限');
...
eq(shusshojiKyufu(MAX, 28, 0).amount, 310290, '【オラクル】産後パパ育休28日の上限 = 310,290円');

// 出生後休業支援給付金 13%・28日
eq(shienKyufu(MAX, 28, 28, false).amount, 60205, '【オラクル】出生後休業支援13%の上限 = 60,205円');
eq(shienKyufu(MIN, 28, 28, false).amount, 11658,
...
eq(rich.daily, 16540, '★年齢に関係なく上限は16,540円（ハ）。45〜60歳の18,220円を使ってはいけない');
eq(rich.capped, true, '上限に張りついたことを画面に出せる');
// もし「45〜60歳」の上限を使ってしまうと、この額になる（＝公表値と矛盾する額）
eq(yen(D.chingin_nichigaku_max.age45_59 * 30 * RATE_HIGH), 366222,
   '（参考）45〜60歳の上限を誤用すると366,222円になり、公表値332,454円と食い違う');
...
throws(() => applyIkujiCaps(10000, { chingin_nichigaku_min: 3203 }), '上限が欠けたデータで計算しない');
...
}
{
  const r = calcPapaIkukyu({
    total6m: 16540 * 180 * 2, // 上限（16,540円）を大きく超える人
    leaveDays: 28, wage: 0, spouse: { exempt: true },
  }, D);
  eq(r.capped, true, '賃金日額が上限に張りつく');
  eq(r.daily, 16540, '★上限は年齢によらず16,540円（17条4項2号ハ・61条の8第4項の読替え）');
  eq(r.shusshoji.amount, 310290, '【オラクル・厚労省】産後パパ育休28日の上限 = 310,290円');
  eq(r.shien.amount, 60205, '【オラクル・厚労省】出生後休業支援28日の上限 = 60,205円');
  eq(r.total, 370495, '上限に張りつく人の合計 = 310,290 + 60,205');
}
